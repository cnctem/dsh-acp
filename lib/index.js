    ...
    if (agentPresets !== undefined) {
      try {
        ...
        const available = presets
          ...
          .map((p) => ({ value: p.id, name: p.name ?? p.id, description: null }))
        if (available.length > 0) {
          configOptions.push({
            ...
            category: 'mode',
            name: 'Mode',
            description: 'Agent preset (tool set and prompt composition)',
            currentValue: record.presetId ?? agentPresets.defaultId,
            options: available,
          })
        }
      } catch {
        // a roster that cannot be read contributes no preset picker
      }
    }
  ...
  const makeAgent = (connection) => {
    ...
    return {
      ...
      async newSession(params) {
        ...
        const defaultPresetId = agentPresets?.defaultId ?? 'standard'
      ...
      async setSessionConfigOption(params) {
        ...
        if (configId === 'model') {
          ...
          if (value === undefined) throw invalidParams('preset requires a string value')
          if (agentPresets === undefined) throw internalError('agent presets are not available')
          if ((record.agent.session.events ?? []).length > 0) {
            throw invalidParams('preset can only be changed before the first prompt')
          }
          const preset = await agentPresets.recompose(record.agent.ctx, value)
      ...
      async loadSession(params) {
        ...
        // Resume under the preset the persisted header recorded (or the default
        // for a preset-less legacy session), so history matches its tool set.
        let presetId = header.agentPreset ?? agentPresets?.defaultId ?? 'standard'
