  ...
  ctx.on('session/event', (session, event) => {
    ...
    switch (event.type) {
      ...
      case 'tool/call': {
        ...
        const locations = toToolCallLocations(args, record.cwd)

        // Snapshot the target file before a mutation runs so we can emit a
        // structured diff on completion.
        if (MUTATING_TOOLS.has(name)) {
          const path = getToolPath(args)
          if (path) {
            try {
              const abs = isAbsolute(path) ? path : resolve(record.cwd, path)
              fileSnapshots.set(callId, { path, oldText: readFileSync(abs, 'utf8') })
            } catch {
  ...
  const makeAgent = (connection) => {
    ...
    return {
      ...
      async loadSession(params) {
        ...
        return { configOptions: [], models: null, modes: null, _meta: {} }
      },
