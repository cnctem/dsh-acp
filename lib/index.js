...
export function apply(ctx, config = {}) {
  ...
  const buildSessionConfiguration = async (record) => {
    ...
    // Model select (drives setSessionConfigOption('model')).
    ...
    // Write permission: the three sandbox/approval presets.
    if (permissionPresets !== undefined) {
      const current = permissionPresets.current(record.agent.session.events)
      configOptions.push({
        type: 'select',
        id: 'permission',
        category: 'permission',
        ...
        options: [...permissionPresets.names].map((name) => ({
          value: name,
    ...
    // Reasoning-effort (thinking) modes.
    let modes = null
    if (llm !== undefined && selection?.provider && selection?.model) {
      try {
        ...
        if (efforts.length > 0) {
          modes = {
            ...
            currentModeId: selection.reasoningEffort ?? info.reasoning.defaultEffort,
          }
        }
      } catch {
        // a model without reasoning metadata contributes no mode picker
      }
    }
  ...
  const makeAgent = (connection) => {
    ...
    return {
      ...
      async newSession(params) {
        ...
        // the host plane, so this agent reads them from the global layer. The
        // mutable model selection is coupled to the agent scope so the client
        // can switch provider/model/reasoning effort at runtime.
      ...
      async setSessionConfigOption(params) {
        ...
        if (configId === 'model') {
          ...
          if (permissionPresets === undefined) throw internalError('permission presets are not available')
