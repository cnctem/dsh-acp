...
export function apply(ctx, config = {}) {
  ...
  const buildSessionConfiguration = async (record) => {
    ...
    if (efforts.length > 0) {
      // A select's currentValue is required by the ACP schema; fall back to
      // the first level when neither the session nor the model names one, so
      // strict clients never drop the option. Display-only: an effort is only
      // passed to the model when the user actually picks it.
      const currentValue = selection.reasoningEffort ?? defaultEffort ?? efforts[0].id
      configOptions.push({
        type: 'select',
        id: 'thought_level',
    ...
    const modes =
      efforts.length > 0
        ? {
            ...
            currentModeId: selection.reasoningEffort ?? defaultEffort ?? efforts[0].id,
          }
        : null
