...
/**
 * Whether a prompt carries content beyond what this bridge accepts. Richer
 * inline payloads (audio, embedded resource) are capabilities this bridge
 * does not advertise, so they are rejected rather than silently dropped.
 * `image` blocks are supported and admitted through dsh's durable attachment
 * seam (see `acpImagesToEncoded`).
 * @param {readonly import('@agentclientprotocol/sdk').ContentBlock[]} prompt
 * @returns {boolean}
 */
function promptHasUnsupportedContent(prompt) {
  return prompt.some(
    (block) => block.type !== 'text' && block.type !== 'resource_link' && block.type !== 'image',
  )
}

/**
 * Convert ACP image blocks to dsh's wire-admission form (`EncodedImageAttachment`):
 * media type plus canonical base64 data — the same contract every dsh RPC
 * endpoint that accepts browser uploads uses, so the bridge reuses the shared
 * `admitEncodedImages` entry instead of re-implementing admission policy.
 * @param {readonly import('@agentclientprotocol/sdk').ContentBlock[]} prompt
 * @returns {{mediaType: string, data: string}[]}
 */
function acpImagesToEncoded(prompt) {
  return prompt
    .filter((block) => block.type === 'image')
    .map((block) => ({ mediaType: block.mimeType, data: block.data }))
}

/** Reject session features outside the automation contract; ignore harmless client-forwarded ones. */
function validateSessionParams(params, logger = { debug() {} }) {
  if (!isAbsolute(params.cwd)) throw invalidParams(`cwd must be an absolute path: ${params.cwd}`)
  if (params.additionalDirectories !== undefined && params.additionalDirectories.length > 0) {
    throw invalidParams('additionalDirectories is not supported')
  }
  // IDE-configured MCP servers (e.g. JetBrains AI Assistant) are forwarded by
  // the client and simply ignored: this bridge exposes no MCP tools, so they
...
/** A short textual stand-in for an image block, for text-only transcript replay. */
function imageMarker(attachment) {
  const ref = attachment ?? {}
  const dims =
    typeof ref.width === 'number' && typeof ref.height === 'number'
      ? `, ${ref.width}x${ref.height} px`
      : ''
  return `[image: ${ref.name ?? ref.mediaType ?? 'attachment'}${dims}]`
}
  ...
  const makeAgent = (connection) => {
    ...
    return {
      ...
      async prompt(params) {
        ...
        let nextImage = 0
        const content = params.prompt.flatMap((block) => {
          switch (block.type) {
            case 'text':
            ...
            case 'image':
              return [{ type: 'image', attachment: imageRefs[nextImage++] }]
            default:
              return []
          }
        })
